...
{								//Début de mon bloc de code.
	setlocale(LC_ALL, ""); //Pour la gestion des charactères accentués. 
	...
	while ((fNombre > 99999) || (fNombre < 10000))  // Tant que le nombre est plus grand que 99999 ou plus petit que 10000 ou que le nombre divisé par 10000 donne moins que 1.
	{
		...
		cin >> fNombre;	//Stock ce que l'utilisateur a entré, dans la variable iNombre.
	} //Passé cette boucle, nous sommes certain que le nombre stocké a 5 chiffres, si la personne a entrée un nombre.
...
Dès qu'il s'agit d'un charactère ou d'une chaîne de charactères, le programme 'plante'.*/
